...
import { VEHICLE_DOSSIER_FOLDERS, validateVehicleForDossier, vehicleDossierFolderName } from "@/lib/drive/business";
...
describe("vehicle Drive dossier rules", () => {
  it("uses the fixed 12-folder Volt & Vroom dossier structure", () => {
    ...
    expect(VEHICLE_DOSSIER_FOLDERS[11]).toBe("12 Garantie & Aftersales");
  });

  it("builds the canonical folder name from plate, vehicle and VIN tail", () => {
    expect(vehicleDossierFolderName({
      id: "veh-1",
  ...
  it("rejects dossiers without id, brand or model", () => {
    expect(() => validateVehicleForDossier({ id: "", brand: "Toyota", model: "Yaris" })).toThrow();
    expect(() => validateVehicleForDossier({ id: "veh", brand: " ", model: "Yaris" })).toThrow();
    expect(() => validateVehicleForDossier({ id: "veh", brand: "Toyota", model: " " })).toThrow();
  });
});
