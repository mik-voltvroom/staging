...
describe("VV CarCheck 101 release logic",()=>{
  ...
  it("rejects a complete inspection with safety-critical failure",()=>{const session=sessionAt();session.checklist=session.checklist.map(i=>({...i,status:"ok" as const}));session.checklist=session.checklist.map(i=>i.id==="brakes_front_pads"?{...i,status:"fail" as const}:i);const result=evaluateCarCheck101(session);expect(result.decision).toBe("REJECTED");expect(result.safetyCriticalRejected).toBe(1);});
  it("requires repair for non-safety attention points",()=>{const session=sessionAt();session.checklist=session.checklist.map(i=>({...i,status:"ok" as const}));session.checklist=session.checklist.map(i=>i.id==="body_paint_variance"?{...i,status:"attention" as const}:i);const result=evaluateCarCheck101(session);expect(result.decision).toBe("REPAIR_REQUIRED");expect(result.releaseBlocked).toBe(true);});
});
