...
export default function InventoryPage() {
  ...
  return <main className="container dashboardPage">
    ...
    <div className="tableWrap"><table><thead><tr><th>Voertuig</th><th>Status</th><th>Bron</th><th>Verkoop</th><th>Marge</th><th>Compleet</th><th>Bijgewerkt</th></tr></thead><tbody>{filtered.map(v => <tr key={v.id}><td><Link className="tableVehicle" href={`/dashboard/voorraad/${v.id}`}><img src={v.images[0] || "https://placehold.co/140x90?text=Geen+foto"} alt=""/><span><strong>{v.brand} {v.model}</strong><small>{v.trim} · {v.year} · {v.mileageKm.toLocaleString("nl-NL")} km</small></span></Link></td><td><span className={`statusPill status-${v.status}`}>{v.status}</span></td><td>{v.source?.provider === "mobilox-hexon" ? "Mobilox" : "VVOS"}</td><td>{eur.format(centsToEuros(v.priceCents))}</td><td className={(v.costs && grossMargin(v.priceCents,v.costs)>=250000)?"marginGood":"marginWarn"}>{v.costs ? eur.format(centsToEuros(grossMargin(v.priceCents,v.costs))) : "–"}</td><td>{v.publication?.completenessPercent ?? 0}%</td><td>{new Date(v.updatedAt).toLocaleDateString("nl-NL")}</td></tr>)}</tbody></table></div>
  </main>;
}
