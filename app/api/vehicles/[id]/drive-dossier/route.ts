...
export async function POST(
  ...
  if (!adminDb) {
    return NextResponse.json(
      { ok: false, error: "Firebase Admin is niet beschikbaar." },
      { status: 503 },
    );
  }
  ...
  if (!snapshot.exists) {
    return NextResponse.json(
      { ok: false, error: "Voertuig niet gevonden." },
      { status: 404 },
    );
  }
  ...
  const vehicle = {
    id: snapshot.id,
    ...snapshot.data(),
  } as Vehicle & { driveDossier?: Record<string, unknown> };

  try {
    validateVehicleForDossier(vehicle);
    if (
      vehicle.driveDossier &&
      typeof vehicle.driveDossier.folderId === "string"
    ) {
      return NextResponse.json({
        ok: true,
        dossier: vehicle.driveDossier,
        reused: true,
      });
    }

    const dossier = await provisionVehicleDossier(
      await getDriveAccessToken(),
      process.env.GOOGLE_DRIVE_VEHICLES_FOLDER_ID!,
      vehicleDossierFolderName(vehicle),
    );
    const driveDossier = {
      folderId: dossier.id,
    ...
    await ref.set(
      { driveDossier, updatedAt: new Date().toISOString() },
      { merge: true },
    );
    return NextResponse.json(
      { ok: true, dossier: driveDossier, reused: false },
      { status: 201 },
    );
  } catch (error) {
    ...
    return NextResponse.json(
      {
        ...
        error:
          error instanceof Error
            ? error.message
            : "Voertuigdossier kon niet worden aangemaakt.",
      },
      { status: 500 },
    );
  }
