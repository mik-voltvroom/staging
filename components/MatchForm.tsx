...
export function MatchForm() {
  ...
  return <form className="matchForm" onSubmit={submit}>
    ...
    {result && <div className={`matchResult ${state}`} aria-live="polite"><span>Uw eerste indicatie</span><h2>{result.title}</h2><p>{result.text} Dit is een eerste richting; wij controleren de keuze persoonlijk op budget, modellen en totale kosten.</p>{state === "success" ? <strong>Uw profiel is ontvangen. Wij nemen persoonlijk contact met u op.</strong> : <strong>Opslaan lukte niet. Bel 06 19 16 35 31 of mail naar sales@voltvroom.nl.</strong>}</div>}
  </form>;
}
