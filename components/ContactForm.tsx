...
export function ContactForm({ vehicles }: { vehicles: { id: string; label: string }[] }) {
  ...
  return <form className="contactForm" onSubmit={submit}>
    ...
    <div className="formGrid publicFormGrid">
      ...
      <label className="span2">Vertel kort hoe je rijdt<textarea name="message" rows={4} placeholder="Bijvoorbeeld: 60 km per dag, vooral snelweg, thuis laden is mogelijk." /></label>
    </div>
    ...
    <p className={`formFeedback ${state}`} aria-live="polite">{state === "success" ? "Gelukt. Je aanvraag is ontvangen; we nemen persoonlijk contact met je op." : state === "error" ? "Versturen lukt nu niet. Bel 050 211 3883 of mail naar mik@voltvroom.nl." : ""}</p>
  </form>;
}
