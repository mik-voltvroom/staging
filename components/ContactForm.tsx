...
export function ContactForm({ vehicles }: { vehicles: { id: string; label: string }[] }) {
  ...
  return <form className="contactForm" onSubmit={submit}>
    ...
    <p className={`formFeedback ${state}`} aria-live="polite">{state === "success" ? "Gelukt. Uw aanvraag is ontvangen; we nemen persoonlijk contact met u op." : state === "error" ? "Versturen lukt nu niet. Bel 050 211 3883 of mail naar mik@voltvroom.nl." : ""}</p>
  </form>;
}
